...
默认入口现为独立分支上的 [Loop 工作台 v0.5](loop-workbench.md)。以下保留原有设计说明。
...
默认入口显示研究室列表，进入研究室后选择目标或自由讨论；任务图位于“关系”视图，选择其他关系是在切换同一份工作的投影。反馈通过新一轮任务表达，不向已经完成的依赖图中加入回边。所有视图共享稳定对象 ID，可互相定位。
...
研究室是长期协作空间，一个研究室可以有多个目标。研究室自由讨论、每个目标的讨论分别保存消息、引用和输入草稿。普通讨论不会自动创建 Goal；用户可点击「发起目标」，也可把已有消息转成目标，保留原消息与证据的来源回链。

发起目标时先输入研究问题，完成标准、预算边界与证据可逐步补齐；未填写的条件明确显示待定，不由界面编造。目标保存研究室上下文的快照。v0.3 原型只可选创建一项固定的「梳理证据与下一步」演示工作，不代表 Agent 已完成分析或开始真实执行。

「补充目标说明」保存研究问题、完成标准与边界的新版本，旧说明可回看。已有工作保留创建时的目标版本，新交办的工作引用更新后的说明；此操作不重新派发任务，也不把旧测量自动视为满足新条件。

暂停目标会暂停其活跃工作；恢复时重新排队，只恢复此次由目标暂停的工作，手动暂停项保持原状。结束研究前需交付或取消未结束的工作，并记录结论；允许结论为没有得到确定结果。人工结束状态与性能验收判定独立。重新开放目标保留历史结论和已经交付的记录。
...
首次打开进入研究室列表，后续恢复上次所在位置。研究室概览展示进行中和已结束的目标，允许先进入自由讨论；进入目标后，顶部呈现目标状态和需要决定的事项。主体是人和 Agent 的研究记录；工作、产物、关系是同一目标的不同视角。消息与工作各有稳定入口，切换视图保留草稿和引用。进入证据或队友详情不丢失讨论位置。
...
v0.3 在共享研究室基础上实现研究室创建、每室多个 Goal、目标独立讨论/工作/证据、讨论转目标及来源回链、目标暂停/恢复/结束/重新开放、任务交付/取消、全局搜索和表单草稿恢复。首次读取兼容迁移 v0.2 本地数据。DSV4 关系页面保留明确标识的示例概览；新目标关系由本地对象生成，原 v0.1 任务图独立保留，状态不互通。
