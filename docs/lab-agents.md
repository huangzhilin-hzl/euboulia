...
agents on the same machine. An agent has a name and a short role description;
its identity is independent of the source repositories it researches. Actual
access is determined by the locally configured runtime.
...
Give the agent a name and a short role description. There is no required
capability list or source-directory picker. The Lab displays a single-use pairing
code valid for ten minutes and a command using the new agent's ID. Copy the
connection and startup commands separately and run them on the agent machine.
Remote SSH instructions expand in the same panel. Closing the panel does not
mark the agent connected; the roster updates when the connector polls the service.
...
The following uses `prism` as a convenient local configuration directory name.
The UI-generated command uses the agent ID; either is valid. No source-directory
argument is needed:
...
### Default workspace and source references

When `--workspace` is omitted, the connector creates a private `workspace/`
directory next to its configuration file. For the example above, that is
`~/.config/euboulia/agents/prism/workspace`. Each agent needs a separate config
directory; its workspace persists when the connector stops and restarts. Existing
contents are retained. A default workspace that is a file or symbolic link is
rejected before consuming the pairing code.

This is a default starting directory, not a repository assignment. Reference
multiple source trees, datasets, and documents in each message or task using
**参考资料**. Paths refer to the agent machine, and links are reference data.
The Lab does not upload, clone, mount, or grant access to those resources.

For advanced setups, append `--workspace /absolute/path/to/existing-directory`
to the connection command to retain a particular starting directory. It must
already exist; its contents and permissions are not changed. Existing connector
configurations keep their recorded workspace and need no re-pairing. Task
references do not override the configured cwd, command, or runtime permissions.

The default directory does not grant write access to the Codex adapter: its
read-only execution policy remains in force. Connector session state and work
records continue to persist independently of source repositories.
...
above. The connector creates its default workspace on that remote machine.
If you opt into `--workspace`, use a remote path. No SSH password or private key
is uploaded to Lab. Keep the Lab, tunnel, and connector running; a sleeping
laptop is offline.
...
The optional **参考资料** field accepts multiple paths or document URLs, one per
line (up to 8,000 characters in total). Each dispatch captures its own references;
changing them for a later task does not rewrite earlier work. References appear
in the work detail panel and are delivered as `job.context.resources`, a string.
Empty references are omitted, including when retrying older requests.
...
The `agent.capabilities` field remains available for older clients and adapters;
it is an empty string for a new identity unless explicitly supplied through the
API. `name` and `role` remain required.
